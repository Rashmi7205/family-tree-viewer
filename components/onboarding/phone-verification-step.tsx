...
export default function PhoneVerificationStep({ onVerified }: PhoneVerificationStepProps) {
  const { user } = useAuth()
      ...
      {!showOtpForm ? (
        ...
        <form onSubmit={otpForm.handleSubmit(onOtpSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="otp" placeholder="123456" maxLength={6} disabled={isLoading} {...otpForm.register("otp")} />
            {otpForm.formState.errors.otp && (
              <p className="text-sm text-red-500">{otpForm.formState.errors.otp.message}</p>
            )}
            <p className="text-xs text-slate-500 dark:text-slate-400">Enter the 6-digit code sent to {phoneNumber}</p>
          </div>
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading && <Icons.spinner className="mr-2 h-4 w-4 animate-spin" />}
            Verify
          ...
          <div className="text-center">
            <Button type="button" variant="link" size="sm" onClick={resendOtp} disabled={isResending || cooldown > 0}>
              {isResending && <Icons.spinner className="mr-2 h-3 w-3 animate-spin" />}
