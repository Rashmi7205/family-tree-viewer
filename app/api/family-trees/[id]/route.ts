...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const relationships = await Relationship.find({ familyTreeId: params.id }).lean()
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to update family tree" }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to delete family tree" }, { status: 500 })
  }
}
